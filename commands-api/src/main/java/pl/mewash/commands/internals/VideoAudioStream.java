...
public enum VideoAudioStream implements DownloadOption {
    ...
    @Getter final static Predicate<Set<DownloadOption>> conflictsPredicate = (options) -> options
        .stream()
        .filter(dOption -> dOption instanceof pl.mewash.commands.settings.formats.VideoQuality)
        .count() > 1;

    public List<CmdEntry> getDownloadCommand() {
        String parameter = switch (this) {
            case MAXIMUM, HIGH, STANDARD, COMPACT -> getForceH264Fallbacks();
        };
        return new LinkedList<>(List.of(CmdEntry.withParam(DownloadCmd.FALLBACKS_CHAIN, parameter)));
    }
